...
public class Predator extends Animal implements Runnable {
    ...
    public int[] random_direction(int X, int Y) {
        ...
        while (true) {
            ...
            if (nextX >= 0 && nextX < World.MAP.length && nextY >= 0 && nextY < World.MAP.length) {
                if (World.MAP[nextX][nextY] != 'C' && World.MAP[nextX][nextY] != 'F'
    ...
    public Prey find_prey() {
        ...
        for (Prey prey : World.preys) {
            if (!prey.isHiding()) {
                distance = (prey.getX() - this.getX()) * (prey.getX() - this.getX())
                        + (prey.getY() - this.getY()) * (prey.getY() - this.getY());
    ...
    public void hunt() {
        ...
        if (prey != null) {
            while (prey.getHealth() > 0 && boredness < 10) {
                while ((prey.getX() - this.getX()) * (prey.getX() - this.getX())
                        + (prey.getY() - this.getY()) * (prey.getY() - this.getY()) > 1) {
                    if (!prey.isHiding()) {
